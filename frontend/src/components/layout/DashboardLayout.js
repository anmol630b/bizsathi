...
const DashboardLayout = ({ children }) => {
  ...
  const SidebarContent = () => (
    <>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--gray-100)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {isMobile && (
          <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--gray-400)', padding: '4px' }}>
            <FiX size={20} />
          </button>
        )}
      </div>
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--gray-100)', background: 'var(--gray-50)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '38px', height: '38px', background: 'linear-gradient(135deg, #E1F5EE, #1D9E75)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '600', fontSize: '14px', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: (planColors[user?.plan] || '#888') + '20', color: planColors[user?.plan] || '#888', padding: '1px 8px', borderRadius: '20px', fontSize: '10px', fontWeight: '600', textTransform: 'uppercase' }}>
              <FiZap size={8} /> {user?.plan || 'free'} Plan
            </div>
          </div>
        </div>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.label}
              {item.label === 'Orders' && (
                <span style={{ marginLeft: 'auto', background: 'var(--danger)', color: 'white', borderRadius: '20px', padding: '1px 7px', fontSize: '10px', fontWeight: '600' }}>New</span>
              )}
            </Link>
          );
        })}
      ...
      <div style={{ padding: '12px', borderTop: '1px solid var(--gray-100)' }}>
        {business?.isPublished && (
          <a href={`http://localhost:3001/store/${business.slug}`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: 'var(--radius)', background: 'var(--primary-light)', color: 'var(--primary)', fontWeight: '600', fontSize: '13px', marginBottom: '4px', textDecoration: 'none' }}>
            <FiGlobe size={16} /> View My Store
          </a>
        )}
        <Link to="/plans" onClick={() => isMobile && setSidebarOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: 'var(--radius)', background: 'linear-gradient(135deg, #E1F5EE, #EEEDFE)', color: 'var(--primary)', fontWeight: '600', fontSize: '13px', marginBottom: '4px', textDecoration: 'none' }}>
          <FiZap size={16} /> Upgrade Plan
        </Link>
        <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: 'var(--radius)', background: 'transparent', border: 'none', color: 'var(--gray-500)', fontSize: '14px', width: '100%', cursor: 'pointer', transition: 'all 0.15s' }}
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--gray-50)', overflow: 'hidden' }}>
      ...
      {!isMobile && (
        <aside style={{ width: '240px', background: 'white', borderRight: '1px solid var(--gray-100)', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100vh', position: 'sticky', top: 0 }}>
          <SidebarContent />
        </aside>
      )}
      ...
      {isMobile && (
        <aside style={{ width: '240px', background: 'white', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 50, transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.3s ease', boxShadow: sidebarOpen ? 'var(--shadow-xl)' : 'none' }}>
          <SidebarContent />
        </aside>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {/* Topbar */}
        <header style={{ background: 'white', borderBottom: '1px solid var(--gray-100)', padding: '0 20px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 30, boxShadow: 'var(--shadow-sm)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {/* Mobile Menu Button */}
            {isMobile && (
              <button onClick={() => setSidebarOpen(true)} style={{ width: '36px', height: '36px', borderRadius: 'var(--radius)', border: '1px solid var(--gray-200)', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--gray-600)' }}>
                <FiMenu size={18} />
              </button>
            )}
            ...
            <span style={{ fontSize: '14px', fontWeight: '600', color: 'var(--gray-500)', borderLeft: '1px solid var(--gray-200)', paddingLeft: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div onClick={() => isMobile && setSidebarOpen(true)} style={{ width: '36px', height: '36px', background: 'linear-gradient(135deg, #E1F5EE, #1D9E75)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '600', fontSize: '14px', cursor: isMobile ? 'pointer' : 'default', flexShrink: 0 }}>
              {user?.name?.charAt(0)?.toUpperCase()}
            </div>
          </div>
        </header>
