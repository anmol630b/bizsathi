...
const StoreFinder = () => {
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        setLocationLoading(false);
        toast.error('Could not detect location. Please enter city manually.');
      },
      { timeout: 10000 }
    );
  };
    ...
    <div style={{ minHeight: '100vh', background: 'white', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #0F172A 0%, #1E293B 100%)', padding: '64px 24px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '16px', marginBottom: '36px', maxWidth: '480px', margin: '0 auto 36px' }}>
            Discover local businesses across India — shops, gyms, restaurants and more
          </p>
          ...
          <form onSubmit={handleSearch} style={{ maxWidth: '640px', margin: '0 auto' }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '8px', display: 'flex', gap: '8px', boxShadow: '0 20px 50px rgba(0,0,0,0.3)' }}>
              ...
              <div style={{ width: '1px', background: '#F1F5F9', margin: '8px 0', flexShrink: 0 }} />
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #F1F5F9', padding: '16px 24px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px' }}>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => { setCategory(cat.id); setTimeout(() => fetchStores(), 100); }} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '99px', border: 'none', background: category === cat.id ? '#0F172A' : '#F8FAFC', color: category === cat.id ? 'white' : '#64748B', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif', whiteSpace: 'nowrap', transition: 'all 0.2s', boxShadow: category === cat.id ? '0 4px 12px rgba(15,23,42,0.2)' : 'none', flexShrink: 0 }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        {searched && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            <div>
              ...
              {userLocation && <p style={{ fontSize: '13px', color: '#94A3B8', marginTop: '2px' }}>Sorted by distance from your location</p>}
              {city && !userLocation && <p style={{ fontSize: '13px', color: '#94A3B8', marginTop: '2px' }}>Stores in {city}</p>}
            </div>
          </div>
        )}
        ...
        {!loading && searched && stores.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            <button onClick={() => { setSearch(''); setCity(''); setCategory('all'); fetchStores(); }} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#0F172A', color: 'white', border: 'none', borderRadius: '12px', padding: '12px 24px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                ...
                <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', border: '1px solid #F8FAFC', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', cursor: 'pointer' }}
                  ...
                  <div style={{ height: '120px', background: `linear-gradient(135deg, ${categoryColors[store.category] || '#00C896'}20, ${categoryColors[store.category] || '#00C896'}40)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                    {store.logo ? (
                      ...
                      <div style={{ width: '72px', height: '72px', borderRadius: '18px', background: categoryColors[store.category] || '#00C896', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: '800', color: 'white', fontFamily: 'Plus Jakarta Sans, sans-serif', boxShadow: '0 8px 20px rgba(0,0,0,0.15)' }}>
                        {store.name.charAt(0)}
                      </div>
                    )}
                  ...
                  <div style={{ padding: '18px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <h3 style={{ fontSize: '16px', fontWeight: '800', color: '#0F172A', fontFamily: 'Plus Jakarta Sans, sans-serif', letterSpacing: '-0.01em', lineHeight: '1.3' }}>{store.name}</h3>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                      ...
                      {store.totalOrders > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: '#94A3B8' }}>
                          <FiShoppingBag size={11} /> {store.totalOrders} orders
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <a href={`https://wa.me/91${store.whatsapp}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ width: '40px', height: '40px', background: 'linear-gradient(135deg, #25D366, #1DAA54)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', boxShadow: '0 4px 10px rgba(37,211,102,0.3)', flexShrink: 0 }}>
        ...
        {!searched && !loading && (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            <h3 style={{ fontSize: '22px', fontWeight: '700', color: '#0F172A', marginBottom: '10px', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
              Discover Local Businesses
            </h3>
            <p style={{ color: '#94A3B8', fontSize: '15px', marginBottom: '28px', maxWidth: '400px', margin: '0 auto 28px' }}>
              Search by city, use your location, or browse by category to find stores near you
            </p>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={handleGetLocation} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#0F172A', color: 'white', border: 'none', borderRadius: '14px', padding: '13px 24px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'Inter, sans-serif', boxShadow: '0 6px 16px rgba(15,23,42,0.2)' }}>
